...
from _1_adversarial_ML.all_attacks import FGSM_Attack
...
def main_sub(
    ...
    # attacking the model
    target = np.ones_like(my_data.x_test[-100:])
    
    my_attack = FGSM_Attack(my_data, my_model)
    
    adversarial_samples = my_attack.attack(
        my_data.x_test[-100:], target, 
        epsilon=0.1, targeted=True
    )
    
    # write metrics code here
    # ...
